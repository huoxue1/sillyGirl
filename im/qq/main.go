...
func init() {
	...
	bot.Client.OnNewFriendRequest(func(qqClient *client.QQClient, request *client.NewFriendRequest) {
		request.Accept()
		log.Infoln(fmt.Sprintf("已同意%v的好友申请，验证信息为：%v", request.RequesterUin, request.Message))
	})
